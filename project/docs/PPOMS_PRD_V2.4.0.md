...
| **功能点** | 1. **创建主单 (`order create`)**：通过命令行参数指定计划月份、标的类别、需求单位和任务名称，自动生成 `CG-xxxx` 格式的主单编号。<br>2. **添加明细 (`order add-item`)**：向指定主单添加明细，支持必填项（名称、规格、数量、单位）和选填项（单价、备注）。<br>3. **智能继承**：CLI 添加明细时，**完全继承 GUI 端的业务逻辑**，包括：<br>&nbsp;&nbsp;- 自动生成递增的序号（如 `2603MPJ-1`）。<br>&nbsp;&nbsp;- 触发自动推荐规则（根据名称匹配采购员、采购方式）。<br>&nbsp;&nbsp;- 触发逻辑联动（如“询比采购”自动匹配“线下采购”）。<br>&nbsp;&nbsp;- 自动设置默认进度要求。 |
| **验收标准** | 1. 在终端运行 `python cli.py order create ...` 成功创建主单。<br>2. 运行 `python cli.py order add-item ...` 成功添加明细，且自动填充字段（如推荐采购员）与数据库规则一致。<br>3. CLI 录入的数据能在 GUI 界面中无缝显示和继续操作。 |
