...
| V2.3.0168 | 2026-01-04 | 蔡勒 | 增强 | 新增合同管理、入库管理及自动化状态流转功能 |
| V2.4.0 | 2026-03-22 | 蔡勒 | 增强 | 新增 CLI 2.0 命令行工具，支持自动化采购计划录入 |
...
- [4. 功能需求 (Functional Requirements)](#4-功能需求-functional-requirements)
    ...
    - [4.8 合同管理 (Contract Management)](#48-合同管理-contract-management)
    - [4.9 入库管理 (Inbound Management)](#49-入库管理-inbound-management)
    - [4.10 命令行接口 (CLI)](#410-命令行接口-cli)
...
#### 4.3.A 明细导入模板更新（新增“备注”列）
- 模板列新增：`备注`（位于最后一列）。
- 备注支持中英文、常用符号与换行显示，导入长度限制为500字符；超过部分自动截断，并在导入提示中显示被截断条数。
- 模板说明页已添加填写规范：采购方式/采购途径的合法取值、计划发放留空或“未分配”表示未分配、备注长度与换行支持。
- 验收标准：包含备注内容的Excel成功导入；长备注被截断且提示统计正确；导入后的备注在明细表中按行高自适应显示。
...
#### 4.5.A 界面布局优化（筛选区上移）
- 将筛选功能区移动至页面顶部，置于“计划月份选择”行的下一行。
- 保留原有筛选条件与组合查询能力（序号、采购标的、主单编号、需求单位）。
- 布局采用水平栅格与自适应伸缩，保持界面风格一致。
- 验收标准：筛选区位置调整后，所有筛选条件仍正常工作，导出与打印均基于筛选结果。
...
### 4.8 合同管理 (Contract Management)

| ID | FR-CONT-001 |
| :--- | :--- |
| **需求名称** | 合同全生命周期管理 |
| **优先级** | P0 |
| **需求描述** | 管理采购合同的签订、执行与结算。 |
| **功能点** | 1. **合同台账**：录入合同基本信息（编号、名称、金额、供应商、附件等）。<br>2. **明细管理**：录入合同包含的物资规格、单价及总数量。<br>3. **订单执行**：基于合同明细开具“执行订单”，自动扣减合同剩余数量。<br>4. **状态跟踪**：自动计算执行金额，展示合同执行进度（百分比）。<br>5. **订单状态**：支持“新增”、“部分入库”、“已入库”三种状态，通过颜色区分（黑/蓝/绿）。 |
| **验收标准** | 1. 开具执行订单时，若数量超过合同剩余量应预警。<br>2. 订单状态应根据入库情况自动流转。<br>3. 支持按合同类别（如“模块”、“线缆”）进行分类管理。 |

### 4.9 入库管理 (Inbound Management)

| ID | FR-INB-001 |
| :--- | :--- |
| **需求名称** | 采购入库与状态联动 |
| **优先级** | P0 |
| **需求描述** | 对已下达的采购订单进行入库登记，并反向更新订单状态。 |
| **功能点** | 1. **入库登记**：选择关联订单，录入本次入库数量、仓储单位及日期。<br>2. **智能选择**：在选择订单弹窗中，已完成（剩余数≤0）的订单自动置灰并禁用，防止超额入库。<br>3. **状态同步**：<br>&nbsp;&nbsp;- 入库数=0 -> 订单状态“新增”<br>&nbsp;&nbsp;- 0<入库数<订单数 -> 订单状态“部分入库”（蓝色）<br>&nbsp;&nbsp;- 入库数>=订单数 -> 订单状态“已入库”（绿色）<br>4. **历史记录**：支持查看入库流水，并提供修改与删除功能（删除后自动回滚订单状态）。 |
| **验收标准** | 1. 添加入库单后，对应合同订单的状态应实时更新。<br>2. 无法选中已完全入库的订单进行再次入库。<br>3. 删除入库记录后，订单状态应准确恢复到之前的状态。 |

### 4.10 命令行接口 (CLI)

| ID | FR-CLI-001 |
| :--- | :--- |
| **需求名称** | 采购计划命令行工具 (CLI 2.0) |
| **优先级** | P2 |
| **需求描述** | 提供基于命令行的无界面操作方式，支持快速创建主单和添加明细，适用于自动化脚本或高级用户。 |
| **功能点** | 1. **创建主单 (`order create`)**：通过命令行参数指定计划月份、标的类别、需求单位和任务名称，自动生成 `CG-xxxx` 格式的主单编号。<br>2. **添加明细 (`order add-item`)**：向指定主单添加明细，支持必填项（名称、规格、数量、单位）和选填项（单价、备注）。<br>3. **智能继承**：CLI 添加明细时，**完全继承 GUI 端的业务逻辑**，包括：<br>&nbsp;&nbsp;- 自动生成递增的序号（如 `2603MPJ-1`）。<br>&nbsp;&nbsp;- 触发自动推荐规则（根据名称匹配采购员、采购方式）。<br>&nbsp;&nbsp;- 触发逻辑联动（如“询比采购”自动匹配“线下采购”）。<br>&nbsp;&nbsp;- 自动设置默认进度要求。<br>4. **进度更新 (`order update-status`)**：支持通过命令行批量更新一条或多条明细的进度状态（如更新为“已完成”）。 |
| **验收标准** | 1. 在终端运行 `python cli.py order create ...` 成功创建主单。<br>2. 运行 `python cli.py order add-item ...` 成功添加明细，且自动填充字段（如推荐采购员）与数据库规则一致。<br>3. 运行 `python cli.py order update-status --status "已完成" --details 2603MPJ-1 2603MPJ-2` 成功更新对应明细的状态。<br>4. CLI 录入的数据能在 GUI 界面中无缝显示和继续操作。 |
...
详见独立文档：[PPOMS_DB_Design_V2.3.0168.md](PPOMS_DB_Design_V2.3.0168.md)
